...
Shell script to install a `Corallium Coin Masternode` on a Linux server running Ubuntu 16.04. Supports IPv4, IPv6 and multiple nodes on a single VPS.
...
This script will work alongside masternodes that were installed by other means provided the masternode binaries `coralliumd` and `corallium-cli` are installed to `/usr/local/bin`.
...
The script assumes you are running a cold wallet on your local PC and this script will execute on a Ubuntu Linux VPS (server). The steps involved are:
 ...
 1. Run the masternode installation script as per the [instructions above](https://github.com/click2install/corallium#installation).
...
You are now ready to configure your local wallet and finish the masternode setup
 ...
 1. Make sure you have downloaded the latest wallet from https://github.com/npq7721/Corallium2/releases
 ...
 5. Unlock your wallet and send **exactly 2,500 CRLM** to the address created in step #4
...
The easiest ports to choose would be 10233 for your first masternode, then 10235, 10237, 10239, etc. The RPC port will be chosen based on your port selection.
...
Once you have setup the 2nd or more masternodes, use the output of the script for each masternode and follow the [steps above](https://github.com/click2install/corallium#how-to-setup-your-masternode-with-this-script-and-a-cold-wallet-on-your-pc) in your wallet, where each new masternode is a new line in your `masternode.conf` file. **NOTE:** All the port numbers in your masternode.conf file will be 10233 or the wallet will not re-start after you save the config file.

Note that multiple masternodes use only one instance of the `coralliumd` and `corallium-cli` binary files located in `/usr/local/bin` and they each have their own configuration located in `/home/<username>/.corallium` folder.
